...
Which icon set renders is governed by the `[render] animation_theme`
field in the active config — see the "Themes" section of
`animation.md`. One icon per signal category per brief — not one per
...
**Audit:** in the rendered output, every ASCII art block (icon, setup
mascot, empty-brief mascot, run-header glyph) has exactly one blank
line of separation on each side. Two or more blank lines, or zero, is
a violation.
...
role as the lane it overrides. The icon names per theme live in
[`../assets/animation.md`](../assets/animation.md); the lane / fires-when
...
mapping in `../assets/animation.md`. New stall patterns map onto the
existing four sub-flavors — no new art, no new palette roles. A
